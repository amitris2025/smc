...
## رفع خطای RE10140 (سقف ۶۴ plot count)

TradingView برای هر اسکریپت حداکثر **۶۴ plot count** مجاز می‌کند؛ عبور از آن خطای
runtime با کد **RE10140** («The script creates too many plots») می‌دهد. برخلاف تصور
رایج، فقط `plot()` و `plotshape()` شمرده نمی‌شوند:

- `plot/plotshape/plotchar/plotarrow/plotbar/plotcandle` بسته به آرگومان‌های سری
  ۱ تا ۷ واحد؛ هر رنگ یا textcolor سری یک واحد اضافه می‌گیرد.
- `alertcondition()`، `bgcolor()` و `barcolor()` هر کدام **۱ واحد**.
- `fill()` فقط وقتی رنگش **series** باشد ۱ واحد (رنگ ثابت رایگان است).
- `hline()`، `line.new()`، `label.new()`، `box.new()` و `table.new()` واحد مصرف
  نمی‌کنند (سقف جداگانهٔ ۵۰۰ عددی با `max_*_count` تنظیم می‌شود).

نسخهٔ قبلی ۲۲ `alertcondition` جدا (یک جفت Buy/Sell برای هر گروه) داشت و با احتساب
وزن‌ها به مرز سقف می‌رسید؛ در نسخهٔ کاربر روی ساختار گسترده‌تر، خطای RE10140 با
۱۱۷ plot count رخ می‌داد. اصلاح انجام‌شده:

- جفت‌های هشدار Buy/Sell هر گروه (Hull، Divergence تأییدشده/کاندید، Fib 1-3،
  Extensionهای 0.300 تا 10.618، Final و KAF/SAGHF) در یک `alertcondition`
  جهت‌دار با شرط OR ادغام شدند: ۲۲ → ۱۱ هشدار.
- شرط کاندید واگرایی طوری بازنویسی شد که امتیاز هر سمت فقط با کاندید همان سمت
  مقایسه شود (متغیرهای امتیاز در کندل بدون پیوت `na` هستند).
- منطق سیگنال‌ها، لیبل‌های `plotshape` روی چارت و پیام هشدار Final دست‌نخورده ماند
  (هر دو مقدار BuyPts/SellPts در پیام می‌آیند).
- شمارندهٔ وزنی `tools/check_plot_budget.py` اضافه شد؛ خروجی فعلی: **۴۹/۶۴**.

برای آزاد کردن واحد بیشتر در آینده، به‌ترتیب کم‌هزینه‌ترین کارها: ادغام بیشتر
هشدارها، تبدیل رنگ‌های سری به ثابت، و استفاده از `line/label/box` (که واحد plot
نمی‌گیرند) به‌جای `plotshape` در گزارش‌های تاریخی.
