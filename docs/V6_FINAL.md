...
|---|---|---|
| plot count (شامل `plot`، `plotshape`، `alertcondition`، `fill` با رنگ series) | 64 | **53** (۱۵ پلات = ۱۷ سهم + ۲۶ plotshape + ۱۰ alertcondition + ۴ fill با رنگ ثابت = ۰ سهم) |
...
خطا می‌دهد. مقادیر تشخیصی به‌جای `plot(display.data_window)` در ردیف‌های
۴۸ تا ۵۲ پنل نمایش داده می‌شوند — جدول هیچ سهمی مصرف نمی‌کند. حذف همان
۱۱ پلات Data Window بود که ۱۱ سهم آزاد کرد و کل ظاهر نسخهٔ v5 (۲۲ شکل
سیگنال + همهٔ خطوط) بدون قربانی‌کردن هشدارها جا شد.

## ۱۳. بازسازی ظاهر نسخهٔ v5 روی موتور v6

سیاست قطعی این نسخه: **ظاهر روی چارت = فیلتر اصلی v5، منطق و دقت = موتور v6.**

### ۱۳٫۱ آنچه عیناً از v5 بازگردانده شد
- تعریف `indicator()` با `precision = 0` و بدون پارامتر `format` (مثل v5).
- ۲۲ شکل سیگنال با همان عنوان، رنگ، اندازه، متن و مکان:
  Final Trend Confluence BUY/SELL، NTS Fib 1-3 BUY/SELL،
  NTS BUY/SELL 0.300 و 1.470 و 2.100، Extension Reversalهای
  2.618/3.000، 4.414/4.764، 6.618/8.618، 10.618، NTS KAF/SAGHF و
  Smart Bullish/Bearish Reversal.
- خطوط چارت: `NTS Stop` با رنگ پویای teal/red/gray و ضخامت ۲، سه
  Retracement اصلی سفید/نارنجی/سبز با ضخامت ۲ و `plot.style_linebr`،
  چهار باند ابر با aqua/fuchsia و دو fill «ناحیه تقاضای ابر»/«ناحیه عرضه ابر»،
  «EQ داخلی روز قبل»، «سقف روز قبل»/«EQ ناحیه روز قبل»/«کف روز قبل» با
  دو fill «Premium روز قبل»/«Discount روز قبل».
- خطوط روند با رنگ `lime`/`red` و لیبل فارسی «روند صعودی ×N» / «روند نزولی ×N»
  با اندازهٔ small (ورودی‌های `tlColUp`/`tlColDn` مثل v5 برگشتند).
- رنگ و ضخامت خطوط Extension از پالت v5 (آرایهٔ `fibColors`): aqua، yellow،
  lime، fuchsia، orange، red، purple؛ نسبت‌های ۴٫۴۱۴ به بالا ضخیم و توپُر،
  بقیه نازک و خط‌چین. خطوط تاریخی خاکستری ۷۲ و نقطه‌چین.

### ۱۳٫۲ فرمول و مجموعهٔ نسبت‌های فیبوناچی (مطابق v5)
```
Retracement : level = extreme + (trail − extreme) × ratio
Extension   : level = fib1 + dir × |trail − fib1| × (ratio − 1)
```
مرجع Extensionها سطح ۰٫۶۱۸ (`fib1`) است و `ratio = 1` خودِ fib1 است؛
۰٫۳۰۰ یعنی ۰٫۷ واحد پشت fib1 و ۲٫۶۱۸ یعنی ۱٫۶۱۸ واحد جلوی آن.
مجموعهٔ نسبت‌ها: ۰٫۶۱۸/۰٫۷۸۶/۰٫۸۸۶ (از ورودی کاربر)، ادامه‌روندهای
۰٫۳۰۰/۱٫۴۷۰/۲٫۱۰۰، بازگشتی‌های ۲٫۶۱۸/۳٫۰۰۰، ۴٫۴۱۴/۴٫۷۶۴،
۶٫۶۱۸/۸٫۶۱۸، ۱۰٫۶۱۸ و دو Retracement عمیق −۰٫۴۶۸/−۰٫۱۶۸ (فقط رسم).
سطوح ۰٫۲۳۶/۰٫۳۸۲ که در v5 وجود نداشتند نگه داشته شدند ولی پیش‌فرض
خاموش‌اند.

### ۱۳٫۳ آنچه از v6 باقی ماند (بدون تغییر ظاهر پیش‌فرض)
- نشانگر `PRO BUY` / `PRO SELL` (قوی‌ترین سطح سیگنال موتور v6).
- نقطهٔ Extreme، روبان Hull، خط Daily Open و لوزی‌های تغییر روند NTS:
  هر چهار مورد پیش‌فرض **خاموش** (`showNtsExtremeDot`, `hullShowRibbon`,
  `showDailyOpenLine`, `showFlipMarks`) تا چارت شلوغ‌تر از v5 نشود.
- Hull همچنان به‌عنوان فیلتر تأیید فعال است و وضعیتش در پنل نمایش داده
  می‌شود؛ فقط رسم روبان خاموش است (دقیقاً مثل v5).
- پلات‌های Data Window حذف شدند؛ همان اعداد در ردیف‌های ۴۸ تا ۵۲ پنل هست.

### ۱۳٫۴ دقت بالاتر، بدون تغییر ظاهر
هر ۲۲ نشانگر حالا از موتور v6 تغذیه می‌شوند، یعنی نسبت به v5 این
دروازه‌ها اضافه شده‌اند: ترتیب اجباری بلوغ موج ← لمس ← واکنش تأییدشدهٔ
بدنه/سایه، cooldown مستقل هر سطح، تفکیک واکنش هم‌جهت/خلاف‌جهت، الزام تأیید
ساختاری یا واگرایی برای سطوح عمیق، امتیازدهی ترکیبی مستقل خرید/فروش با سه
آستانهٔ SETUP/CONFIRMED/PRO، هم‌راستایی سه تایم‌فریم بالاتر، جاروی
نقدینگی جهت‌دار، و قفل‌کردن کندل بسته‌شدهٔ تایم‌فریم پایه برای حذف
ری‌پینت. نشانگر «Final Trend Confluence» فقط روی سطح CONFIRMED به بالا
روشن می‌شود.
